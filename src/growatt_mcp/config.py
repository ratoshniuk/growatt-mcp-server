...
TOKEN_HELP = "Get your token in the ShinePhone app: Me > tap your username > API Token."
...
def load_settings(env: Mapping[str, str] | None = None) -> Settings:
    ...
    ``GROWATT_TOKEN`` is required. ``GROWATT_BASE_URL`` overrides the API host directly;
    otherwise ``GROWATT_REGION`` (``global``, ``eu``, ``cn`` or ``us``) selects one of the
    documented regional hosts. ``GROWATT_TIMEOUT`` is the HTTP timeout in seconds.
    """
    env = os.environ if env is None else env

    token = env.get("GROWATT_TOKEN", "").strip()
    if not token:
        raise ConfigError(f"GROWATT_TOKEN environment variable is required. {TOKEN_HELP}")
    ...
    if timeout <= 0:
        raise ConfigError("GROWATT_TIMEOUT must be positive.")

    return Settings(token=token, base_url=base_url.rstrip("/"), timeout=timeout)
